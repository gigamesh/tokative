...
const AFFILIATE_CODE_RE = /^aff-[a-z2-9]{8}$/;
...
export default function AffiliateLandingPage() {
  ...
  const isValid = AFFILIATE_CODE_RE.test(code);
  const [loadingPlan, setLoadingPlan] = useState<string | null>(null);

  useEffect(() => {
    if (isValid) {
      localStorage.setItem("tokative_aff", code);
    }
  }, [code, isValid]);
    ...
    <div className="min-h-content bg-surface">
      <div className="max-w-2xl mx-auto px-4 py-20 text-center space-y-8">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-accent-cyan-muted/20 border border-accent-cyan-muted-half">
          <Handshake className="w-8 h-8 text-accent-cyan-text" />
        </div>

        <div className="space-y-3">
          <h1 className="text-3xl font-bold text-foreground">
            Welcome from a {APP_NAME} Partner
          </h1>
          <p className="text-lg text-foreground-muted">
            Manage your TikTok engagement at scale with{" "}
            <span className="text-accent-cyan-text font-semibold">
              {APP_NAME}
            </span>
            . Collect comments, send bulk replies, and track everything in one
            dashboard.
          </p>
        </div>
